...
// T is the element type, so each thread copies exactly one element. A float copy over f16/i16 data
// moves 4 bytes at a 2-byte stride: it also writes the next element, racing that element's own writer
// at the src0/src1 seam, and writes 2 bytes past the end of dst on the last element.
template<typename T>
